...
**Forward** — Screen A attaches the context to the push itself (scoped to *this* push, no globals to clean up):

```swift
// ScreenAFeature (SwiftUI + TCA)
navigator.push(.screenC(ScreenCContext(
    subtitle: "Pushed from Screen A",
    output: { store.send(.screenC($0)) }   // ← the return channel
)))
```
...
sequenceDiagram
    ...
    Routes-->>A: store.send(.screenC(.textSubmitted("hello")))
...
3. **Data round-trip** — on Screen A, push Screen C **with context**: C shows A's subtitle; type a message, hit **Send**, pop back — Screen A is displaying what you typed in C.
